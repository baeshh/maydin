...
배포 사이트(예: `https://maydin.co.kr`)에서 **카카오맵 API 도메인 제한** 때문에 지도가 안 나올 수 있습니다. 아래 순서대로 하면 해결됩니다.
...
- 저장 후 1~2분 지나서 **https://maydin.co.kr** 새로고침
...
| 2 | 플랫폼 → Web → 사이트 도메인에 `https://maydin.co.kr` + (로컬 테스트 시) `http://localhost:8080` 입력 후 저장 |
...
2. **리다이렉트 URI 등록**  
   - **카카오 로그인** → **Redirect URI**에 다음 주소를 등록 후 저장  
     - 배포: `https://maydin.co.kr/kakao-callback.html`  
